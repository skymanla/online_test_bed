...
//개수
//$count = "select count(*) as cnt from tbl_device_booking where 1 $whereis group by booking_seqnum";
$count = "select count(distinct booking_seqnum) as cnt from tbl_device_booking where 1 $whereis";
...
$sql = "select *, count(a.booking_seqnum) as seq_cnt from 
		tbl_device_booking a join tbl_member c on a.booking_mseq=c.mseq 
		where 1 $whereis
		group by a.booking_seqnum
		having count(a.booking_seqnum) > 0
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td class="txt_c"><a href="s1view.php?idx=<?=$row['booking_seqnum']?>">[<?=$sys_cate?>] <?=$row['booking_dv_name']?><?=$etc_count?></a></td>
